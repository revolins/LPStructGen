...
class ElphHashes(object):
    ...
    def _get_intersections(self, edge_list, hash_table):
        ...
        #logger.info(f"******** MEMORY ENTERING INTERSECTIONS FOR LOOP  ***********")
        #process = psutil.Process() #DEBUG
        #logger.info(process.memory_info().rss / (10**9))
        for k1 in range(1, self.max_hops + 1):
            for k2 in range(1, self.max_hops + 1):
                # logger.info("********** TYPE HLL ************") # DEBUG
                # logger.info(type(hash_table[k1]['hll']))
                # logger.info("********** TYPE MINHASH ************")
                # logger.info(type(hash_table[k1]['minhash']))
                # logger.info("********** EDGELIST SIZE ************")
                # logger.info(edge_list.size()) # DEBUG
                src_hll = hash_table[k1]['hll'][edge_list[:, 0]]
                #logger.info(f"******** MEMORY ENTERING INTERSECTIONS AFTER HLL = 0  ***********")
                #process = psutil.Process() #DEBUG
                #logger.info(process.memory_info().rss / (10**9))
                src_minhash = hash_table[k1]['minhash'][edge_list[:, 0]]
                #logger.info(f"******** MEMORY ENTERING INTERSECTIONS AFTER MINHASH = 0  ***********")
                #process = psutil.Process() #DEBUG
                #logger.info(process.memory_info().rss / (10**9))
                dst_hll = hash_table[k2]['hll'][edge_list[:, 1]]
                dst_minhash = hash_table[k2]['minhash'][edge_list[:, 1]]
                #logger.info(f"******** MEMORY ENTERING JACCARD (K_1 = {k1}, K_2 = {k2})  ***********")
                #process = psutil.Process() #DEBUG
                #logger.info(process.memory_info().rss / (10**9))
                jaccard = self.jaccard(src_minhash, dst_minhash)
                #logger.info(f"******** MEMORY ENTERING MERGE (K_1 = {k1}, K_2 = {k2})  ***********")
                #process = psutil.Process() #DEBUG
                #logger.info(process.memory_info().rss / (10**9))
                unions = self._hll_merge(src_hll, dst_hll)
                #logger.info(f"******** MEMORY ENTERING COUNT (K_1 = {k1}, K_2 = {k2})  ***********")
                #process = psutil.Process() #DEBUG
                #logger.info(process.memory_info().rss / (10**9))
    ...
    def get_subgraph_features(self, links, hash_table, cards):
        ...
        if not self.use_zero_one:
            if self.max_hops == 2:  # for two hops any positive edge that's dist 1 from u must be dist 2 from v etc.
                ...
                features[:, 9] = 0 #https://github.com/melifluos/subgraph-sketching/blob/main/src/hashing.py#L310
                features[:, 10] = 0
                features[:, 11] = 0
                features[:, 12] = 0
