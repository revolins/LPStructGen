...
class HashDataset(Dataset):
    ...
    def _generate_file_names(self, num_negs, data_name):
        ...
        if num_negs == 1 or self.split != 'train':
            subgraph_cache_name = f'dataset/{data_name}Dataset/{self.root}{self.split}{year_str}{end_str}'
        else:
            subgraph_cache_name = f'dataset/{data_name}Dataset/{self.root}{self.split}_negs{num_negs}{year_str}{end_str}'
...
def make_train_eval_data(args, train_dataset, num_nodes, n_pos_samples=5000, negs_per_pos=1000):
    ...
    #Hard-Coded replace with ROOT_DIR?
    if args.fnr == True:
        subgraph_cache_name = f'dataset/{dataset_name_save}/train_eval_negative_samples_{negs_per_pos}_edgeproposalset_subgraph_featurecache.pt'
    else:
        subgraph_cache_name = f'dataset/{dataset_name_save}/train_eval_negative_samples_{negs_per_pos}_subgraph_featurecache.pt'
    ...
    if os.path.exists(subgraph_cache_name):
        ...
        if args.max_hash_hops == 3:
            hash_name = f'dataset/{dataset_name_save}/{file_stub}_elph__train_3hop_hashcache.pt'
        else:
            hash_name = f'dataset/{dataset_name_save}/{file_stub}_elph__train_hashcache.pt'
