...
def get_ppr_matrix(edge_index, num_nodes, alpha=0.15, eps=5e-5):
    ...
    print("# Nodes in Dataset:", edge_index.max().item())
...
def get_ppr(data, alpha=0.15, eps=1e-5):
    """
    Wrapper
    """
    root_dir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "..", "..")
    dataset_dir = os.path.join(root_dir, "node_subsets", "ppr", data['dataset'])
    file_name = f"sparse_adj-{alpha}_eps-{eps}".replace(".", "") + ".pt"
    sparse_adj_file_path = os.path.join(dataset_dir, file_name)

    if os.path.isfile(sparse_adj_file_path):
        print("PPR matrix exists. Reading from file...")
        sparse_adj = torch.load(sparse_adj_file_path)
    else:
        ...
        save_results(data['dataset'], sparse_adj, alpha, eps)
