...
from util.read_datasets import read_data
...
def train_loop(args, train_args, data, device, loggers, seed, model_save_name, verbose):
    ...
    for epoch in range(1, 1 + args.epochs):
        ...
        if epoch % args.eval_steps == 0:
            ...
            results_rank = test(model, score_func, data, evaluator_hit, evaluator_mrr, args.test_batch_size)
...
def train_data(args, train_args, device, verbose=True):
    """
    ...
    for seed in tqdm(range(0, args.runs), f"Training over {args.runs} seeds"):
        ...
        if "ogbl" in  args.data_name:
            data = read_data(args, 1, device)
        else:
            data = read_data(args, seed+1, device)
