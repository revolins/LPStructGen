...
# import wandb
from BUDDY.hashdataset import get_hashed_train_val_test_datasets, make_train_eval_data
...
def get_loaders(args, dataset, splits, directed):
    ...
    train_loader = dl(train_dataset, batch_size=args.train_bsz,
    ...
    shuffle_val = False if args.dataset.startswith('ogbl-citation') else True
    val_loader = dl(val_dataset, batch_size=args.eval_bsz, shuffle=shuffle_val,
                    num_workers=args.num_workers)
    shuffle_test = False if args.dataset.startswith('ogbl-citation') else True
    test_loader = dl(test_dataset, batch_size=args.eval_bsz, shuffle=shuffle_test,
                     num_workers=args.num_workers)
    if (args.dataset == 'ogbl-citation2') and (args.model in {'ELPH', 'BUDDY'}):
        train_eval_loader = dl(
            make_train_eval_data(args, train_dataset, train_data.num_nodes,
                                  n_pos_samples=5000), batch_size=args.train_bsz, shuffle=False,
            num_workers=args.num_workers)
    else:
...
def get_loaders_hard_neg(args, dataset, splits, directed):
    ...
    val_loader = dl(val_dataset, batch_size=args.eval_bsz, shuffle=False,
                    num_workers=args.num_workers)
    
    test_loader = dl(test_dataset, batch_size=args.eval_bsz, shuffle=False,
                     num_workers=args.num_workers)
