import torch
from BUDDY.model import BUDDY, ELPH
...
def select_model(args, dataset, emb, device):
    ...
    split_list = ['cn', 'pa', 'sp']
    if 'ff' in args.dataset.lower() or any(splt_t in args.dataset.lower() for splt_t in split_list):
        data = dataset.get()
    else:
        data = dataset[0]

    if args.model == 'ELPH':
        #Currently hard-coded to 128
        if 'ppa' in args.dataset.lower() and any(splt_t in args.dataset.lower() for splt_t in split_list):
            model = ELPH(args, 58, node_embedding=emb).to(device)
        else:
            model = ELPH(args, 128, node_embedding=emb).to(device)
    else:
        if 'ppa' in args.dataset.lower() and any(splt_t in args.dataset.lower() for splt_t in split_list):
            model = BUDDY(args, 58, node_embedding=emb).to(device) # Hidden features hard-coded to match data.x
        else:
            model = BUDDY(args, 128, node_embedding=emb).to(device)
