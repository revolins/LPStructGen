...
from BUDDY.gnn import SIGNEmbedding, SIGN
...
class BUDDY(torch.nn.Module):
    ...
    def __init__(self, args, num_features=None, node_embedding=None):
        ...
        self.label_dropout = args.en_dp
        self.feature_dropout = args.en_dp
        ...
        if self.sign_k != 0:
            if self.propagate_embeddings:
                ...
                self.sign_embedding = SIGNEmbedding(args.n_hidden, args.n_hidden, args.n_hidden,
                                                    args.sign_k, args.en_dp, args.edge_drop)
            else:
                self.sign = SIGN(num_features, args.n_hidden, args.n_hidden, args.sign_k,
                                 args.en_dp, args.edge_drop)
        self.label_lin_layer = Linear(self.dim, self.dim)
        if args.use_feature:
            self.bn_feats = torch.nn.BatchNorm1d(args.n_hidden)
        if self.node_embedding is not None:
            self.bn_embs = torch.nn.BatchNorm1d(args.n_hidden)
        ...
        if args.use_feature:
            self.lin_feat = Linear(num_features,
                                   args.n_hidden)
            self.lin_out = Linear(args.n_hidden, args.n_hidden)
        hidden_channels = self.dim + args.n_hidden if self.use_feature else self.dim
        if self.node_embedding is not None:
            self.lin_emb = Linear(args.n_hidden,
                                  args.n_hidden)
            self.lin_emb_out = Linear(args.n_hidden, args.n_hidden)
    ...
    def feature_forward(self, x):
        ...
        if self.sign_k != 0:
            ...
            x = self.lin_feat(x)
        temp_x = F.cosine_similarity(x[:, 0, :],  x[:, 1, :])
        x = x[:, 0, :] * x[:, 1, :]
        # mlp at the end
        x = self.lin_out(x)
        x = self.bn_feats(x)
    ...
    def forward(self, sf, node_features, src_degree=None, dst_degree=None, RA=None, emb=None):
        ...
        if self.use_feature:
            node_features, temp_x = self.feature_forward(node_features)
        ...
        if self.use_RA:
            ...
            x = torch.cat([x, RA], 1)
        x = self.lin(x)
        return x, temp_x # This will break if self.use_features is not equal to true but that should never be the case
    ...
    def reset_parameters(self):
        self.label_lin_layer.reset_parameters()
        self.lin_feat.reset_parameters()
        self.lin_out.reset_parameters()
        self.lin_emb.reset_parameters()
        self.lin_emb_out.reset_parameters()


class LinkPredictor(torch.nn.Module):
    def __init__(self, args, no_sf, use_embedding=False):
        ...
        self.bn_labels = torch.nn.BatchNorm1d(self.dim)
        if args.use_feature:
            self.lin_feat = Linear(args.n_hidden,
                                   args.n_hidden)
            self.lin_out = Linear(args.n_hidden, args.n_hidden)
        ...
        if self.no_sf:
            ...
            out_channels = self.dim + args.n_hidden if self.use_feature else self.dim
...
class ELPH(torch.nn.Module):
    ...
    def __init__(self, args, num_features, node_embedding=None):
        ...
        self.dim = args.max_hash_hops * (args.max_hash_hops + 2)
        self.no_sf = args.no_sf_elph
        # construct the nodewise NN components
        self._convolution_builder(num_features, args.n_hidden,
                                  args)  # build the convolutions for features and embs
        # construct the edgewise NN components
        self.predictor = LinkPredictor(args, self.no_sf, node_embedding is not None)
