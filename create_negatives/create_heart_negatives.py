...
def calc_all_heuristics(args, seed):
    """
    Calc and store top-k negative samples for each sample
    """
    dataset = SynthDataset(f"{args.dataset}_seed{seed}", root_dir=args.root_dir)
    print("Getting Data...")
    data = dataset.get()
    print("Getting Edge Split...")
    edge_split = dataset.get_edge_split()
    print("Prepping data...")
    data = prep_data(data, edge_split)

    # Get unique nodes in train
    train_nodes = set(edge_split['train']['edge'].flatten().tolist())

    dataset_dir = os.path.join(ROOT_DIR, "negatives", dataset.dataset_name)
    ...
    print("\n>>> Valid")
    val_neg_samples = rank_and_merge_edges(edge_split['valid']['edge'], cn_scores, pa_scores, data, train_nodes, args)
    save_samples(val_neg_samples, os.path.join(dataset_dir, f"heart_valid_samples.npy"))

    print("\n>>> Test")
    test_neg_samples = rank_and_merge_edges(edge_split['test']['edge'], cn_scores, pa_scores, data, train_nodes, args, test=True)
